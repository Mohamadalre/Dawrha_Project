  ...
  item: [
    ...
    },
    {
      ...
      item: [
        req('📄 عرض العروض', 'GET', 'api/v1/admin/waste/offers?status=all&page=1&limit=10', {
          description:
            'الرد منظَّم بأقسام: `audience` (النوع، الأدوار، وهل يشمل كل النوع)،' +
            ' `grade` (الحالة بالمعرّف والكود، أو null)، `effect` (المقدار، النسبة' +
            ' المشتقّة، والاتجاه INCREASE للبائعين / DECREASE للمشترين مع وصف)،' +
            ' و`validity` (from/until و`is_live` محسوبة على الساعة).',
        }),
        req('➕ إضافة عرض — للبائعين بمقدار ⚙️', 'POST', 'api/v1/admin/waste/offers', {
          body: {
          ...
          description:
            '**لا يوجد عرض «لكل الأدوار».** هناك جمهوران فقط:\n' +
            '- **`SELLERS`** = مواطن + مؤسسات (يُدفع لهم)، فالمقدار **يُضاف** إلى سعرهم.\n' +
            '- **`BUYERS`** = معمل + جهة حرّة (يدفعون)، فالمقدار **يُنقَص** من سعرهم.\n\n' +
            '`amount` **مقدار لا سعراً نهائياً**: العرض الواحد يصل دورين مسعَّرين' +
            ' بشكل مختلف، فلا سعر نهائي واحد يصحّ لكليهما.\n\n' +
            '**البائع لا يُقيَّد بحالة** — يُدفع له قبل الفرز، ولا مانع أن يتجاوز المقدار' +
            ' السعر (زيادة كريمة)، لكن سعر ما بعد العرض لا ينزل أبداً تحت سعر ما قبله.\n\n' +
            '`target_roles` اختياري: إغفاله يعني **كلا دوري الجمهور**؛ وذكر دور من الجهة' +
            ' الأخرى يُرفَض. ⚙️ يدفع صفحة أسعار المادة إلى أودو فوراً.',
        }),
        req('➕ إضافة عرض — للبائعين بنسبة', 'POST', 'api/v1/admin/waste/offers', {
          body: {
            product_id: '{{productId}}',
            audience: 'SELLERS',
            target_roles: ['INSTITUTIONS'],
            percentage: 15,
          },
          description:
            '`percentage` **بديل عن `amount`** (إرسالهما معاً يُرفَض). يُشتقّ منه المقدار' +
            ' مقابل سعر كل شريحة، ويُخزَّن «النوع = نسبة» فيُعاد حساب المقدار كلما تحرّك' +
            ' السعر لاحقاً — بينما عرض المقدار يبقي مقداره.',
        }),
        req('➕ إضافة عرض — للمشترين، مادة بلا حالات', 'POST', 'api/v1/admin/waste/offers', {
          body: { product_id: '{{productId}}', audience: 'BUYERS', amount: 2 },
          description:
            'مادة **بلا حالات** ← مقدار واحد بلا `conditions`. ويُرفَض المقدار إن ساوى' +
            ' السعر أو تجاوزه (يجعله صفراً أو سالباً = تدفع للمشتري ليأخذ المادة).',
        }),
        req('➕ إضافة عرض — للمشترين بالحالات (مقدار لكل حالة) ⚙️', 'POST', 'api/v1/admin/waste/offers', {
          body: {
            product_id: '{{gradedProductId}}',
            audience: 'BUYERS',
            conditions: [
              { condition_id: '{{conditionId}}', amount: 3 },
              { condition_id: '{{conditionId2}}', amount: 2 },
            ],
          },
          description:
            'معمل/جهة حرّة يشترون **بالحالة**، فالمقدار يُذكَر لكل حالة في `conditions[]`' +
            ' **بالمعرّف** (الكود فريد داخل مادته فقط). وكلها لازم تتبع **نفس المادة**؛' +
            ' والمفتاح الأجنبي يمنع حذف حالة يسمّيها عرض حيّ (DELETE يُردّ بـ23001).\n\n' +
            'كل حالة تُفحَص وحدها ضد سعرها، وتُشتقّ نسبتها من سعرها هي.',
        }),
        req('➕ إضافة عرض — للمشترين بنسبة على كل الحالات ⚙️', 'POST', 'api/v1/admin/waste/offers', {
          body: { product_id: '{{gradedProductId}}', audience: 'BUYERS', percentage: 25 },
          description:
            'مادة مصنَّفة **بلا ذكر حالة** + نسبة ← تُطبَّق على **كل حالة** ويُشتقّ لكل' +
            ' واحدة مقدارها من **سعرها هي**: ٢٥٪ من حالة سعرها ٧٠ = ١٧٫٥، ومن ٦٠ = ١٥.' +
            ' (ولو أُرسل `amount` بدل النسبة، طُبّق نفسه على كل حالة ورُفض إن أنزل أرخصها' +
            ' تحت الصفر — والرسالة تسمّي الحالة العاجزة.) ونسبة المشتري لا تبلغ ١٠٠٪.',
        }),
        req('✏️ تعديل عرض ⚙️', 'PUT', 'api/v1/admin/waste/offers/{{offerId}}', {
          body: {
            amount: 3,
            target_roles: ['FACTORY'],
            valid_until: '2026-12-31T00:00:00.000Z',
          },
          description:
            'قواعد الحالة والجمهور تُعاد هنا كما في الإنشاء. الحقل `amount` (لا `offer_price`)،' +
            ' و**النسبة تُعاد اشتقاقها في كل تعديل** لأن الخصم يتوقف على المقدار والحالة' +
            ' والشرائح، وهذا الراوت يحرّك أياً منها. ⚙️',
        }),
        req('⏱️ تعديل مدة العرض ⚙️', 'PATCH', 'api/v1/admin/waste/offers/{{offerId}}/validity', {
          body: { valid_until: '2026-12-31T23:59:59.000Z' },
          description:
            'راوت مستقلّ لتمديد عرض يوشك أن ينتهي دون إعادة إرسال المقدار والجمهور.\n\n' +
            'التاريخ **بالساعة والثانية** (timestamp)، فتنتهي المدة في لحظتها بالضبط.' +
            ' و`valid_until: null` تعني **مفتوح حتى إشعار آخر**.\n\n' +
            '⚙️ يصل أودو فوراً: تقصير المدة يُنهي العرض، وصفحة أودو تكفّ عن شطب السعر' +
            ' في اللحظة نفسها (has_offer محسوبة على الساعة).',
        }),
        req('💲 تعديل مقدار العرض + التاريخ ⚙️', 'PATCH', 'api/v1/admin/waste/offers/{{offerId}}/amount', {
          body: {
            amount: 4,
            valid_until: '2026-10-15T12:30:00.000Z',
          },
          description:
            '`amount` (لا `offer_price`)، والتاريخان اختياريان **في نفس الطلب** لأنهما قرار' +
            ' واحد؛ فصلهما يترك العرض حيّاً بالمقدار الجديد على التواريخ القديمة لحظةً.\n\n' +
            '**الطلبيات المنفَّذة لا تتأثر**: سطر الطلب يُثبّت السعر لقطةً عند إنشائه.' +
            ' والمقدار يُعاد التحقق منه كالإنشاء (يُرفَض إن جعل السعر صفراً أو سالباً)،' +
            ' والنسبة تُعاد اشتقاقها. ⚙️',
        }),
        req('⭐ مفضّلة حساب (قراءة فقط)', 'GET', 'api/v1/favourites/account/{{accountId}}', {
          description:
            'النافذة **الوحيدة** للأدمن على المفضّلة: يقرأ مفضّلة حساب مُعيَّن ولا يضيف ولا' +
            ' يعدّل ولا يحذف — والثلاثة مرفوضة له صراحةً.\n\n' +
            'المفضّلة كالسلة: مجموعة عمل خاصة بالمشتري لا يكتب فيها أحد سواه. و`cart.view`' +
            ' وحدها لا تعبّر عن ذلك لأن الأدمن يملك كل مفاتيح الكتالوج، فالأدوار مسمّاة' +
            ' صراحةً في كل راوت.',
        }),
        req('🗑️ حذف عرض ⚙️', 'DELETE', 'api/v1/admin/waste/offers/{{offerId}}', {
          description:
            'أودو يعلم بالحذف عبر **غياب** العرض عند القراءة التالية — وهو نفس مسار انتهاء' +
            ' المدة تماماً. سلوك واحد لا اثنان، فالسطر يعود إلى سعر القائمة في الحالتين.',
        }),
      ],
    },
    {
    ...
    },
    {
      ...
      item: [
        ...
        req('♻️ إعادة فتح طلب مرفوض', 'POST',
            'api/v1/account-management/{{accountId}}/reopen', {
          body: { reason: 'سنعيد النظر في طلبك' },
          description:
            '**وثائق الطلب المرفوض مجمَّدة.** القرار اتُّخذ و**أُرسِل**، وإعادة وسم الأدلة' +
            ' تحته تغيّر *ما رُفض المتقدّم بسببه* بعد أن أُبلِغ به — ولا شيء على أي من' +
            ' الشاشتين يقول إن الأسباب تحرّكت.\n\n' +
            'فإعادة النظر مسموحة، لكن **في العلن**: هذا الراوت يعيد الطلب إلى' +
            ' `PENDING_APPROVAL`، فتعود الوثائق قابلة للتعديل، **ويُبلَّغ المتقدّم** أن طلبه' +
            ' يُنظَر فيه من جديد. آخر ما سمعه كان رفضاً؛ إعادة النظر بصمت تتركه لا يعلم أن' +
            ' عليه انتظار جواب.\n\n' +
            '**بابٌ واحد لا بابان**: طلبُ وثيقة من حساب مرفوض كان سيصير إعادة فتح من الباب' +
            ' الخلفي — يُسحَب المتقدّم إلى `NEED_CHANGES` دون أن يُقال له إن الرفض يُعاد' +
            ' النظر فيه. لذلك `request-reupload` يرفض الآن على المرفوض حتى يُعاد فتحه.\n\n' +
            '**وملاحظة المراجع تبقى** — هي سبب الرفض الأول، وتلك السابقة هي ما يجعل' +
            ' النظرة الثانية ذات معنى.\n\n' +
            '**409** إن لم يكن الطلب مرفوضاً.\n\n' +
            'ونفس الفعل في أودو للسائق: زرّ **«إعادة فتح الطلب»** (`action_reactivate`).',
        }),
