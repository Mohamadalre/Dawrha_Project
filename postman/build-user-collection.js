      ...
      item: [
        ...
        req('🎁 العروض', 'GET', 'api/v1/user-app/guest/offers?page=1&limit=10', {
          ...
          description: d(
            ...
            '`offer_id` · `product_id` · `product_name` · `product_image` ·',
            '`offer_price` · `discount_percentage` · `condition` · `valid_from` · `valid_until`.',
            '',
            '**من**: `offers` ⋈ `products`، بشرط `is_active` و`valid_from <= NOW()`',
            'و`valid_until` فارغة أو مستقبلية، و**استهداف الشريحة**: عرضٌ موجَّه للمصانع',
            'لا يظهر في تطبيق المستخدم — الإعلان عن خصم لا يمكن للقارئ أخذه أسوأ من عدم الإعلان.',
          ),
        }),
    ...
    // ══════════════════════════════════════════════════════════════════
    {
      ...
      item: [
        req('👤 ملفي', 'GET', 'api/v1/user/profile', {
          description: d(
            '**يرجع**: `{ message, result: { account, profile } }`',
            '- `account` — صفّ `accounts` كاملاً **ما عدا `passwordHash`** (يُحذف قبل الإرجاع).',
            '- `profile` — صفّ `citizen_profiles` مع علاقاته (العناوين).',
            '',
            '**من**: `accounts` + `citizen_profiles` (+ `locations` + `provinces`).',
          ),
        }),
        req('✏️ تعديل الملف', 'PATCH', 'api/v1/user/profile', {
          body: { name: 'محمد أحمد', phone: '+963931234567', description: 'نبذة قصيرة' },
          description: d(
            '**يأخذ** (كلها اختيارية): `name` ≤255 · `phone` (`+` ثم ٧–١٥ رقماً) · `description` ≤1000.',
            '',
            ...
            '',
            '**يكتب**: `accounts` (الحقول المرسَلة فقط).',
          ),
        }),
      ...
      item: [
        req('🎁 العروض', 'GET', 'api/v1/waste/offers?page=1&limit=10&active_only=true&sort=discount', {
          description: d(
            ...
            '',
            '**يرجع**: `{ offers[], pagination }` وكل عرض:',
            '`offer_id` · `product_id` · `product_name` · `product_image` · `offer_price` ·',
            '`discount_percentage` · `condition` · `description` · `valid_from` · `valid_until`.',
            '',
            '**من**: `offers` ⋈ `products`، مع **استهداف الأدوار**: العرض الموجَّه',
            'لشريحة أخرى لا يظهر للمواطن.',
          ),
        }),
      ...
      item: [
        ...
        req('➕ إضافة مادة', 'POST', 'api/v1/cart/items', {
          ...
          description: d(
            ...
            '- `add_offer` — اختياري: يستخدم سعر العرض إن كان نافذاً.',
            '',
            '**يرجع**: السلّة كاملة بعد الإضافة.',
            '',
            '**يكتب**: صفّاً في `cart_items` وفيه `unit_price` **محسوباً من `product_pricing`**',
            'بشريحة المُنادي. والسعر المفقود **يُرفض** لا يُصفَّر: مادة بلا سعر لهذه الشريحة',
            'مادةٌ لم يكن يُفترض أن يراها أصلاً.',
          ),
        }),
        ...
        req('🎁 إضافة عرض', 'POST', 'api/v1/cart/offers', {
          ...
          description: d(
            ...
            '',
            '**يكتب** بنداً في `cart_items` بـ `is_offer = true` وسعره من `offers.offer_price`',
            'بعد التأكّد أن العرض **نافذ الآن** و**موجَّه لدور المُنادي**.',
          ),
        }),
